...
[SWF (backgroundColor = "0xFFFFFF", frameRate = "30", width = "1400", height = "800")]
public class ShallowWaterDemoMT extends Sprite
{
...
//        landscape.geometry.subGeometries[0].updateVertexData(newVerts);
		...
		// Touch Debugger
		//_touchDebugger = new TouchDebugger();
		//_touchDebugger.init(stage);
    ...
    private function stage_mouseDownHandler(event:MouseEvent):void
    {
        _pressing = true;
         ...
         updateMouse(stage.mouseX,stage.mouseY);
    ...
    private function assignTouch(event:TouchEvent):void {
      touches[event.touchPointID] = event; 
	    _pressing = true;
         updateMouse(event.stageX,event.stageY);
    ...
    private function removeTouch(event:TouchEvent):void {
    ...
    private function enterFrameHandler(event:Event):void
    {
        ...
        for (var key:String in touches) {
          ...
          if (touch) {         
				updateMouse(touch.stageX, touch.stageY);
				trace("updating mouse" + touch.stageX+touch.stageY);
				}
			}
		}
    ...
    private function updateMouse(newX:int, newY:int ):void
    {
        _projX = 1 - 2*newX/stage.stageWidth;
        _projY = 2*newY/stage.stageHeight - 1;
